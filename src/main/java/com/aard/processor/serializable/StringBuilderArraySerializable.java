 ...
 */
public class StringBuilderArraySerializable {
    /**
     * 类型名称
     */
    public static final String CLASS_NAME = StringBuilder[].class.getName();
     ...
     */
    public static byte[] serializable(StringBuilder[] value) {
     ...
     * @date 2021/11/6 9:11
     */
    public static int valueByteLength(StringBuilder[] value) {
        if (value == null) {
            return 1;
        }
        int len = 5;
        for (StringBuilder val : value) {
            len += StringBuilderSerializable.valueByteLength(val);
        }
        return len;
    }
     ...
     * @date 2021/10/20 18:35
     */
    public static byte[] serializableValue(StringBuilder[] value) {
        if (value == null) {
            return new byte[]{0};
        }
        ...
        int index = 5;
        for (StringBuilder val : value) {
            index = StringBuilderSerializable.serializableValue(bytes, index, val);
        }
        return bytes;
    }
     ...
     * @date 2021/10/20 18:35
     */
    public static void serializableValue(StringBuilder[] value, ByteBuf buffer) {
        if (value == null) {
            buffer.writeByte(0);
            return;
        }
     ...
     * @date 2021/10/20 18:35
     */
    public static int serializableValue(byte[] bytes, int index, StringBuilder[] value) {
        if (value == null) {
     ...
     * @date 2021/10/20 18:35
     */
    public static StringBuilder[] deserialization(ByteBuffer byteBuffer) {
        int len = byteBuffer.getInt();
        StringBuilder[] arr = new StringBuilder[len];
        for (int i = 0; i < len; i++) {
            arr[i] = StringBuilderSerializable.deserializationValue(byteBuffer);
        }
        return arr;
    }
     ...
     * @date 2021/10/20 18:35
     */
    public static StringBuilder[] deserializationValue(ByteBuffer byteBuffer) {
