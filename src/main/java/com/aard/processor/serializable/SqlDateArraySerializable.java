 ...
 */
public class SqlDateArraySerializable {
    /**
     * 类型名称
     */
    ...
    public static int valueByteLength(Date[] value) {
        ...
        int len = 5;
        for (Date val : value) {
            len += SqlDateSerializable.valueByteLength(val);
        }
        return len;
    }
    ...
    public static byte[] serializableValue(Date[] value) {
        ...
        int index = 5;
        for (Date val : value) {
            index = SqlDateSerializable.serializableValue(bytes, index, val);
        }
        return bytes;
    }
    ...
    public static Date[] deserialization(ByteBuffer byteBuffer) {
        ...
        for (int i = 0; i < len; i++) {
            arr[i] = SqlDateSerializable.deserializationValue(byteBuffer);
        }
        return arr;
    }
