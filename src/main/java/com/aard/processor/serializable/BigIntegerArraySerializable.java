 ...
 */
public class BigIntegerArraySerializable {
    /**
     * 类型名称
     */
     ...
     * 类型标识
     */
    public static final byte TYPE = 48;
    ...
    public static int valueByteLength(BigInteger[] value) {
        ...
        for (BigInteger val : value) {
            len += BigIntegerSerializable.valueByteLength(val);
        }
        return len;
    }
    ...
    public static byte[] serializableValue(BigInteger[] value) {
        ...
        int index = 5;
        for (BigInteger val : value) {
            index = BigIntegerSerializable.serializableValue(bytes, index, val);
        }
        return bytes;
    }
    ...
    public static BigInteger[] deserialization(ByteBuffer byteBuffer) {
        ...
        for (int i = 0; i < len; i++) {
            arr[i] = BigIntegerSerializable.deserializationValue(byteBuffer);
        }
        return arr;
    }
