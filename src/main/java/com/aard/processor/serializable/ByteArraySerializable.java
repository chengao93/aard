 ...
 */
public class ByteArraySerializable {
    /**
     * 类型名称
     */
    ...
    public static int valueByteLength(Byte[] value) {
        ...
        int len = 5;
        for (Byte val : value) {
            len += ByteSerializable.valueByteLength(val);
        }
        return len;
    }
    ...
    public static byte[] serializableValue(Byte[] value) {
        ...
        int index = 5;
        for (Byte val : value) {
            index = ByteSerializable.serializableValue(bytes, index, val);
        }
        return bytes;
    }
    ...
    public static Byte[] deserialization(ByteBuffer byteBuffer) {
        ...
        for (int i = 0; i < len; i++) {
            arr[i] = ByteSerializable.deserializationValue(byteBuffer);
        }
        return arr;
    }
