 ...
 */
public class CharacterArraySerializable {
    /**
     * 类型名称
     */
    ...
    public static int valueByteLength(Character[] value) {
        ...
        for (Character val : value) {
            len += CharacterSerializable.valueByteLength(val);
        }
        return len;
    }
    ...
    public static byte[] serializableValue(Character[] value) {
        ...
        int index = 5;
        for (Character val : value) {
            index = CharacterSerializable.serializableValue(bytes, index, val);
        }
        return bytes;
    }
    ...
    public static Character[] deserialization(ByteBuffer byteBuffer) {
        ...
        for (int i = 0; i < len; i++) {
            arr[i] = CharacterSerializable.deserializationValue(byteBuffer);
        }
        return arr;
    }
